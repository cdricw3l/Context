...
import { SelectedView } from '../utils/types';
...
const FileContentDetails: React.FC<FileContentDetailsProps> = ({
  ...
  const { selectedViews, setSelectedViews, aggregatedContent, setAggregatedContent } = useResponse();
  const selectedView = selectedViews[fileName] || 'all';
  ...
  const handleChangeView = (view: string) => {
    setSelectedViews((prev) => ({ ...prev, [fileName]: view as SelectedView }));
  };
  ...
  useEffect(() => {
    setAggregatedContent((prev) => ({ ...prev, [fileName]: contentToDisplay || '' }));
  }, [contentToDisplay, fileName, setAggregatedContent]);
    ...
    <div className="p-4 shadow-2xl rounded-xl text-gray-800 mb-4">
      <div className="flex justify-between">
        ...
        <div className="space-x-2">
          <button onClick={() => onCopy(isMinimizedView ? fileContentMinimized : contentToDisplay || '')} className="px-2 py-1 rounded">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-5">
              <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 7.5V6.108c0-1.135.845-2.098 1.976-2.192.373-.03.748-.057 1.123-.08M15.75 18H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08M15.75 18.75v-1.875a3.375 3.375 0 0 0-3.375-3.375h-1.5a1.125 1.125 0 0 1-1.125-1.125v-1.5A3.375 3.375 0 0 0-3.375 7.5H6.75" />
              <path strokeLinecap="round" strokeLinejoin="round" d="M11.25 10.5h.008v.008H11.25z" />
            </svg>
          </button>
      ...
      <div className="flex space-x-4 mb-4">
        <button
          className={`p-2 rounded ${selectedView === 'all' ? 'text-green-500' : ''}`}
          onClick={() => handleChangeView('all')}
        >
          All
        </button>
        <button
          className={`p-2 rounded ${selectedView === 'Import / Export' ? 'text-green-500' : ''}`}
          onClick={() => handleChangeView('Import / Export')}
        >
          Import / Export
        </button>
        <button
          className={`p-2 rounded ${selectedView === 'css' ? 'text-green-500' : ''}`}
          onClick={() => handleChangeView('css')}
        >
          CSS
        </button>
      </div>
      ...
      {isModalOpen && (
        <Modal
          ...
          content={isMinimizedView ? fileContentMinimized : contentToDisplay}
          onClose={handleCloseModal}
          onCopy={() => onCopy(isMinimizedView ? fileContentMinimized : contentToDisplay || '')}
        />
      )}
    </div>
  );
